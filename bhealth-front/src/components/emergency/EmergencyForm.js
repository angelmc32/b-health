...
import { createEmergency, editEmergency } from '../../services/emergency-services';
import 'moment/locale/es';  // without this line it didn't work
moment.locale('es');

const EmergencyForm = ({ url, action }) => {
  ...
  const handleSubmit = (event) => {
    ...
    if ( action === 'create' ) {
      ...
      .then( res => {           // Success -> notification and send to new consultation view,
        ...
        UIkit.notification({  // Send UIkit success notification
          message: '<p class="uk-text-center">La urgencia fue creada exitosamente</p>',
          pos: 'bottom-center',
          status: 'success'
        });
        resetForm();
        setState( prevState => ({...prevState, isButtonDisabled: false, spinnerState: false}))
        push({pathname: `${url}/ver`, state: {consultation: emergency}})
    ...
    if ( action === 'update' ) {
      editEmergency(consultation._id, form)
      .then( res => {           // Success -> notification and send to new consultation view,
        const { emergency } = res.data;
        UIkit.notification({
          message: '<p class="uk-text-center">La urgencia fue actualizada exitosamente</p>',
          pos: 'bottom-center',
          status: 'success'
        });
        setState( prevState => ({...prevState, isButtonDisabled: false, spinnerState: false}));
      ...
      <Fragment>
        ...
        <form onSubmit={handleSubmit} className="uk-form-stacked uk-text-left uk-container">
          <div className="uk-margin">
            ...
            <label className="uk-form-label" htmlFor="form-stacked-text">Lugar de visita:</label>
            <div className="uk-form-controls uk-margin-small-bottom">
          ...
          <div className="uk-width-1-1 uk-flex uk-flex-center uk-margin-medium">
            ...
            { !state.spinnerState ? "Crear Urgencia" : "Creando"}  <div className={ state.spinnerState ? 'uk-visible' : 'uk-hidden'} uk-spinner="true"></div>
            </button>
          </div>
        </form>
      </Fragment>
        ...
        <form onSubmit={handleSubmit} className="uk-form-stacked uk-text-left uk-container">
          ...
          <div className="uk-width-1-1 uk-flex uk-flex-center uk-margin-medium">
            <button type="submit" className="uk-button uk-button-primary uk-button uk-border-pill uk-width-2-3 uk-width-1-4@m uk-margin" disabled={state.isButtonDisabled} >
              { !state.spinnerState ? "Editar Urgencia" : "Actualizando"}  <div className={ state.spinnerState ? 'uk-visible' : 'uk-hidden'} uk-spinner="true"></div>
            </button>
          </div>
        </form>
      </Fragment>
