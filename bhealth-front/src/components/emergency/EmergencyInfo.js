...
const ConsultationInfo = ({ url, action }) => {
  ...
  const [ state, setState ] = useState({
    ...
    spinnerState: false,
    errorMsg: 'Ha ocurrido un error, intente de nuevo',
    consultation: {
      doctor: '',
      doctor_specialty: '',
      date: '',
  ...
  return (
    <div className="uk-container">
      <h2>{action === 'read' ? 'Ver Urgencia' : 'Eliminar Urgencia'}</h2>
      { action === 'read' ?
          ...
          !state.spinnerState ?
            <div className="uk-width-1-1 uk-flex uk-flex-column uk-flex-middle uk-margin-large-bottom">
              <div className="uk-width-1-1">
                <p>¿Estás seguro que deseas eliminar la siguiente urgencia?</p>
              </div>
      ...
      <div className="uk-child-width-1-3@m uk-child-width-1-1 uk-grid-small uk-grid-match uk-margin" uk-grid="true">
        ...
        { state.consultation.treatment ? (
            <button className="uk-button uk-button-default uk-border-pill uk-margin" onClick={event => {push({pathname: '/recetas/ver', state: {prescriptionID: state.consultation.treatment} }); console.log(state.consultation) }} >
              Ver Receta
            </button>
          ) : (
            <button className="uk-button uk-button-default uk-border-pill uk-margin" onClick={event => push({pathname: '/recetas/crear', state: {consultation: state.consultation}}) } >
              Agregar Receta
            </button>
          )
        }
